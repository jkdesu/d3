...
function barChart(data, columnInUse, fillColor){
    ...
    var svg = d3.select("#interactive").append("svg").attr("width",400).attr("height",400)

    var yScale = d3.scaleLinear().domain([0,20]).range([0,300])

    svg.append("text").text(columnInUse).attr("x",10).attr("y",20).attr("font-weight","bold")

    svg.selectAll("rect")
    .data(data)
    .enter()
    .append("rect")
    .attr("y",function(d){return 350-yScale(d[columnInUse])})
    .attr("width",18)
